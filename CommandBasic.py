...
async def ping(ctx):
    ...
    message = await ctx.send(embed=nextcord.Embed(title="Pinging...", color=0xff00ff))
    elapsed_time = time.time() - start_time
    embed = nextcord.Embed(title="Pong!", color=0x00ff00)
    embed.add_field(name="Latency", value=f"{elapsed_time*1000:.0f}ms", inline=True)
    embed.add_field(name="API Latency", value=f"{bot.latency*1000:.0f}ms", inline=True)
...
async def help(ctx):
    ...
    MyEmbed.set_author(name = "Ali Nabilah Ramadhan", url = "https://twitter.com/ampaskopi1126", icon_url = "https://scontent-cgk1-2.cdninstagram.com/v/t51.2885-19/299848341_791344618882809_3466426253983729388_n.jpg?stp=dst-jpg_s150x150&_nc_ht=scontent-cgk1-2.cdninstagram.com&_nc_cat=109&_nc_ohc=6w6fHNQ92s8AX8d3K-5&edm=ACWDqb8BAAAA&ccb=7-5&oh=00_AfC0jcQfWqInjwreD_GYEsteUARoh-s-6KAASBEs-9Ivcg&oe=6404F454&_nc_sid=1527a3")
    MyEmbed.set_thumbnail(url = "https://i.imgur.com/EaCCLfz.png")
    MyEmbed.add_field(name = "!ping", value = "This Command for test ur connection", inline=False)
    MyEmbed.add_field(name = "!coinflip", value = "This Command lets u flip a coin", inline=False)
    MyEmbed.add_field(name = "!rps", value = "This Command allows u to play a game of rock pape scissors with the bot", inline=False)
...
async def mute(ctx, user: nextcord.Member):
    ...
    embed = nextcord.Embed(title="Mute Command", description=f"{user} has been muted.", color=0x00ff00)
...
async def unmute(ctx, user: nextcord.Member):
    ...
    embed = nextcord.Embed(title="Unmute Command", description=f"{user} has been unmuted.", color=0x00ff00)
    await ctx.send(embed=embed)
...
async def deafen(ctx, user: nextcord.Member):
    ...
    try:
        ...
        embed = nextcord.Embed(title="Deafen Command", description=f"{user} has been deafened.", color=0x00ff00)
        await ctx.send(embed=embed)
...
async def undeafen(ctx, user: nextcord.Member):
    ...
    try:
        ...
        embed = nextcord.Embed(title="Undeafen Command", description=f"{user} has been undeafened.", color=0x00ff00)
        await ctx.send(embed=embed)
    except nextcord.Forbidden:
...
async def voicekick(ctx, user: nextcord.Member):
    ...
    try:
        ...
        embed = nextcord.Embed(title="Voice Kick Command", description=f"{user} has been kicked from voice channel.", color=0x00ff00)
        await ctx.send(embed=embed)
    except nextcord.Forbidden:
        await ctx.send("I do not have permission to kick this user from the voice channel.")
    except nextcord.HTTPException:
        await ctx.send("Kicking the user from the voice channel failed. Please try again later.")

# This Bot for Playing Music for You
@bot.command()
async def join(ctx):
...
async def play(ctx, *, searchword):
        ...
        #Downloads the Audio File with the Title, it is run in a different thread so that the bot can communicate to the nextcord\ server while downloading
        def download(url):
            with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
